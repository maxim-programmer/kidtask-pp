...
export default {
  name: "SeniorLayout",
  data() { return { menuOpen: false, balance: 0, avatarUrl: null } },
  computed: {
  ...
  methods: {
    async loadMe() {
      ...
      try {
        ...
        this.avatarUrl = r.data.user?.avatar_url || null
      } catch {}
    },
...
.avatar { width: 36px; height: 36px; border-radius: 50%; background: #6366f1; color: #fff; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 16px; cursor: pointer; overflow: hidden; flex-shrink: 0; }
...
.dropdown__item { display: block; width: 100%; padding: 12px 16px; border: none; background: transparent; text-align: left; font-size: 15px; color: #e5e7eb; cursor: pointer; transition: background 0.15s; font-family: inherit; }
